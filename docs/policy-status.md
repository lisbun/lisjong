...
| `TerminalShantenProgressionMechanismRiichiDefensePolicy` | — | **experimental offensive-efficiency candidate; not evaluated; not promoted** | exact `mechanism-riichi-defense` parentの all-zero completion branch だけを expected terminal shanten 最小化へ置換 |
...
strength 評価はまだ実施しておらず、Arena での 3x passive tsumogiri 400-game development evaluation は別の Arena child Issue で pre-register する。positive な 400-game development result だけでは baseline へ自動昇格しない。
...
| `TerminalShantenProgressionMechanismRiichiDefensePolicy` | [lisjong #169](https://github.com/lisbun/lisjong/issues/169) — experimental candidate; strength evaluation not yet run |
