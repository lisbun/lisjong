...
| `Kobalab0004ReferencePolicy` | — (lisjong identity `kobalab-0004-tile-efficiency-reference-v1`) | **external documented reference; not in strength hierarchy** | kobalab/majiang-ai legacy 0004 の選択規則を lisjong exact shanten へ適用した参照実装（鳴きなし・聴牌即リー・オリなし）。upstream との全局面の行動同値性は主張せず、評価結果はこの移植 Policy の成績として扱う（原実装・RiichiLab「牌効率くん」の成績ではない）。詳細は [kobalab 0004 reference](kobalab-0004-reference.md)（[#211](https://github.com/lisbun/lisjong/issues/211)） |
