...
| --- | --- | --- | --- | --- |
| `lisjong-dev` | 開発・smoke test・protocol調査用 | `LISJONG_DEV_BOT_TOKEN` | `TwoStepUkeirePolicy` | `lisjong-dev` |
...
切り替えた。Issue #58では同向聴・同受け入れ候補を2段階受け入れで比較する
`TwoStepUkeirePolicy`を追加し、`lisjong-dev`をこのPolicyへ切り替えている。
Issue #76ではlegalな`RiichiAction`を優先するAlways Riichi behaviorを同Policyへ
追加した。`lisjong-baseline`は比較基準として固定するという完了条件の
...
profileから外れたPolicyも`lisjong.policies`の公開Policyとして保持する。
`MinimalPolicy`(Policy境界と決定性のbaseline)、`ShantenPolicy`(打牌後向聴数だけを
最小化)、`UkeirePolicy`(最小向聴候補を受け入れ枚数で比較)、
`TwoStepUkeirePolicy`(同向聴・同受け入れ候補を2段階受け入れで比較)、
`GenbutsuDefenseTwoStepUkeirePolicy`(非聴牌かつ他家リーチ時に共通現物を優先)は、
後続のPolicy比較でそのまま並べられる世代として併存させる。守備なしbaselineで
ある`TwoStepUkeirePolicy`と守備ありPolicyの差を保つため、Issue #78ではruntime
profile mappingを変更しない。
