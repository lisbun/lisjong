...
`docs/policy-input-schema.md`の「Materialized state」、`docs/architecture.md`の
RiichiEnv Adapter責務を実装する。1つの`self_seat`視点について、seat-visible
`Observation.new_events()`を継続的に処理し、`PolicyInput`生成に履歴が必要な
公開意味状態だけを保持する。
...
(`docs/riichienv-investigation.md`の「kakan元pon解決とmeld公開状態」)で、
`Observation.melds`がkakanのin-place更新を含む現在snapshotを直接提供する
ことを確認したため、meldはdecisionごとに`Observation.melds`から直接構築する
(`policy_input.py`を参照)。このtrackerでmeldを二重管理しない。
...
(`docs/riichienv-investigation.md`の「event重複防止に関する追加実測」)、
「新しいObservationを受け取るたびに、その`new_events()`全体を1回だけ適用する」
という運用規則そのものをここで守る。
"""
