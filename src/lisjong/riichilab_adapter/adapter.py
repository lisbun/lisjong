"""1 game x 1 seatへbindされたRiichiLab request_action Adapter。

parsed済みの`request_action`相当dataを受け取り、既存#23の`build_decision()`と
#34の`execute_policy()`を再利用して、送信前possible_actions semantic
validationまで完了した送信可能payloadを返す。

```text
RiichiLab request_action
    -> Observation.deserialize_from_base64()
    -> build_decision()            (Issue #23)
    -> execute_policy()            (Issue #34)
    -> validated canonical InternalAction
    -> paired mapping.resolve()
...
schedulerはこのmoduleの責務ではない(Issue #39)。
"""
...
from lisjong.riichienv_adapter.action_mapping import RiichiEnvActionMappingSession
from lisjong.riichienv_adapter.decision import build_decision
from lisjong.riichienv_adapter.materialized_state import SeatMaterializedState
from lisjong.riichienv_adapter.seat_conversion import seat_from_player_index
from lisjong.riichilab_adapter.errors import SeatMismatchError
from lisjong.riichilab_adapter.mjai_response import build_mjai_response
from lisjong.riichilab_adapter.possible_action_validation import (
    validate_against_possible_actions,
)
from lisjong.riichilab_adapter.request_action import parse_request_action
...
class SendReadyResponse:
    ...
    後続Issue #39のWebSocket Clientは、この`action`をJSON化してそのまま
...
class RiichiLabSeatAdapter:
    """1 game x 1 seatへ明示的にbindされたRiichiLab request_action Adapter runtime。
